...
export default class PauseCommand extends BaseCommand<Rhea> {
  ...
  async executeCommand(context: Rhea, interaction: CommandInteraction) {
    ...
    if (!permission) return await interaction.followUp('You must either be alone in the channel, have a role named `DJ` or have the permission `Move Members` to pause the bot.');
    if (!player.paused) await player.pause();
    await interaction.followUp('⏸️ **Paused**');
  }
}
